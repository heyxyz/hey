...
describe("GET /sitemap/posts.xml", () => {
  test("should return 200 and generate a sitemap with post links", async () => {
    ...
    for (let i = 1; i <= totalBatches; i++) {
      expect(data).toContain(
        `<loc>https://api.hey.xyz/sitemap/posts/${i}.xml</loc>`
      );
    }
  });
  ...
  test("should return 500 if an error occurs", async () => {
    ...
    try {
      await axios.get(`${TEST_URL}/sitemap/posts.xml`);
    } catch (error: any) {
