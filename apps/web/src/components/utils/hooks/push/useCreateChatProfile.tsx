import { generateRandomPassword } from '@components/Messages/Push/helper';
...
const useCreateChatProfile = () => {
  ...
  const [password, setPassword] = useState<string>(generateRandomPassword(12));
  ...
  const reset = useCallback(() => {
    ...
    const autoGeneratedPass = generateRandomPassword(12);
  ...
  const handleSetPassword: handleSetPassFunc = useCallback(async () => {
    ...
    let account = `nft:eip155:${CHAIN_ID}:${LENSHUB_PROXY}:${currentProfile.id}`;
    if (
      connectedProfile &&
