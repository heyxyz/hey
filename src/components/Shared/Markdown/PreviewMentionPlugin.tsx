...
import { $createPreviewMentionNode, PreviewMentionNode } from './PreviewMentionNode';
...
export function PreviewMentionPlugin(): JSX.Element | null {
  const [editor] = useLexicalComposerContext();

  useEffect(() => {
    if (!editor.hasNodes([PreviewMentionNode])) {
      throw new Error('PreviewMentionPlugin: PreviewMentionNode not registered on editor');
    }
  }, [editor]);

  const createPreviewMentionNode = useCallback((textNode: TextNode): PreviewMentionNode => {
    return $createPreviewMentionNode(textNode.getTextContent());
  }, []);
  ...
  useLexicalTextEntity<PreviewMentionNode>(getHashtagMatch, PreviewMentionNode, createPreviewMentionNode);
