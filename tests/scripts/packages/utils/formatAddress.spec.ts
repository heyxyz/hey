...
test.describe('formatAddress', async () => {
  test('should return an empty string if null is passed in', async () => {
    const result = formatAddress(null);
    await expect(result).toBe('');
  });

  test('should match the expected format and shortens middle characters with ellipsis', async () => {
  ...
  test('should match the expected format with 5 on each side and shortens middle characters with ellipsis', async () => {
